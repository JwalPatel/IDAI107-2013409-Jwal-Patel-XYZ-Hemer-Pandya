...
def get_energy_recommendations(user_data, appliance_data):
    ...
    try:
        ...
        Analyze ALL listed appliances and provide recommendations in strict JSON format. 
        Include specific insights for each appliance listed.
        Ensure the response starts with {{ and ends with }}. Include:
        1. Current energy analysis
        2. Per-appliance breakdown
        ...
        {{
            "analysis": {{
                ...
                "regional_comparison": "15% above average",
                "peak_usage": "2-5 PM weekdays",
                "carbon_footprint": 280.5
            }},
            "appliance_analysis": [
                {{
                    "name": "ApplianceName",
                    "current_consumption": 180.5,
                    "current_cost": 900.25,
                    "potential_savings_kwh": 45.5,
                    "potential_savings_money": 227.5,
                    "optimization_steps": ["Step1", "Step2"],
                    "upgrade_suggestions": "Suggestion text",
                    "roi_months": 24
                }}
            ],
            "recommendations": [
                {{
                    ...
                    "annual_savings_kwh": 540.0,
                    "annual_savings_money": 2700.0,
            ...
            "renewable_suggestions": [
                {{
                    ...
                    "initial_cost": 150000.0,
                    "annual_production": 3600.0,
                    "annual_savings": 18000.0,
                    "payback_period_years": 8.3,
                    "incentives_available": "30% tax credit"
                }}
            ],
            "target_kwh": 380.5,
            "target_money": 1900.25,
            "estimated_carbon_reduction": 85.5
        }}
        
        Ensure all numeric values are actual numbers (not strings) and arrays are properly formatted."""
...
def init_session_state():
    ...
    # Initialize badges as a dictionary
    ...
    # Ensure badges is a dictionary even if it exists
    elif not isinstance(st.session_state.badges, dict):
        st.session_state.badges = {
            "Weekend Off": False,
            "Night Saver": False,
            "Power Optimizer": False,
...
def main(auth=None):
        ...
        try:
            ...
            if not saved_data.empty:
                ...
                if isinstance(data, list) and len(data) > 0:
                    data = data[-1]  # Get the most recent entry
                
                # Update session state if data is dictionary
                if isinstance(data, dict):
                    # Update appliances
                    if 'appliances' in data:
                        st.session_state.appliances = data['appliances']
                    ...
                    if 'user_level' in data:
                        st.session_state.user_level = data['user_level']
                    
                    # Update badges
                    if 'badges' in data and isinstance(data['badges'], dict):
                        st.session_state.badges.update(data['badges'])
                        
                    # Update historical data if present
                    if 'historical_data' in data:
                        st.session_state.historical_data = pd.DataFrame(data['historical_data'])
                else:
                    st.warning("No valid saved data found")
        except Exception as e:
        ...
        if st.session_state.recommendations:
            ...
            with st.container():
                ...
                <div class='modern-card'>
                    ...
                    <ul>
                        <li><strong>Total Consumption:</strong> {:.1f} kWh/month</li>
                        <li><strong>Regional Comparison:</strong> {}</li>
                        <li><strong>Peak Usage Time:</strong> {}</li>
                        <li><strong>Carbon Footprint:</strong> {:.1f} kg CO₂/month</li>
                    </ul>
                </div>
                """.format(
                    st.session_state.recommendations["analysis"]["total_consumption"],
                    st.session_state.recommendations["analysis"]["regional_comparison"],
                    st.session_state.recommendations["analysis"]["peak_usage"],
                    st.session_state.recommendations["analysis"]["carbon_footprint"]
                ), unsafe_allow_html=True)
    ...
    with tab3:
        ...
        if not isinstance(st.session_state.historical_data, list) and hasattr(st.session_state.historical_data, 'empty') and not st.session_state.historical_data.empty:
            ...
            current_month_idx = min(2, len(data) - 1)  # Simulate being in the 3rd month of tracking
