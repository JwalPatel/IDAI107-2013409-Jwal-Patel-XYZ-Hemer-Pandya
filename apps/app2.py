...
def init_session_state():
    ...
    if 'initialized' not in st.session_state:
        ...
        st.session_state.streaks = 0  # Initialize streaks
    ...
    required_vars = {
        ...
        'user_level': "Energy Novice",
        'points': 0,
        'streaks': 0,  # Add streaks to required vars
        'quiz_completed': False,
        'monthly_targets': {'current': 0, 'target': 0}
    }
    
    for var, default_value in required_vars.items():
        if var not in st.session_state:
            setattr(st.session_state, var, default_value)
