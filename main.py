...
def show_dashboard():
    ...
    # Save metrics both locally and to GitHub
    try:
        ...
        # GitHub save
        try:
            auth.db.github_storage.update_eco_metrics_csv(
                st.session_state.user,
                eco_score,
                metrics['carbon_saved'],
                metrics['water_saved'],
                metrics['energy_saved']
            )
        except AttributeError:
            st.warning("GitHub storage sync temporarily unavailable")
